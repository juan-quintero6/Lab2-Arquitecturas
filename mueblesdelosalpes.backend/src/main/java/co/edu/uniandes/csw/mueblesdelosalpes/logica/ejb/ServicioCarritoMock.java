...
public class ServicioCarritoMock implements IServicioCarritoMockRemote, IServicioCarritoMockLocal
{
    ...
    /**
     * Actualiza un mueble en el carrito de compras
     * @param mueble Mueble con los datos actualizados
     */
    @Override
    public void actualizarItem(Mueble mueble) {
        for (int i = 0; i < inventario.size(); i++) {
            Mueble item = inventario.get(i);
            if (item.getReferencia() == mueble.getReferencia()) {
                // Actualiza los datos del mueble en el carrito
                item.setNombre(mueble.getNombre());
                item.setPrecio(mueble.getPrecio());
                item.setCantidad(mueble.getCantidad());
                // Puedes actualizar otros campos según tus necesidades

                // Recalcula el inventario
                recalcularInventarioTotal();
                return;
            }
        }
    // Si el mueble no se encuentra en el carrito, puedes manejarlo como un error o simplemente no hacer nada.
    }
